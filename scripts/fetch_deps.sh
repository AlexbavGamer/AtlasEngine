...
# Assimp's CMake build normally generates config.h and revision.h. AtlasEngine
...
# revision.h is normally generated by CMake from revision.h.in. Premake does
# not perform that configure step, so provide the version values required by
# Assimp's Version.cpp. GitVersion=0 intentionally means "no generated commit
# hash"; the version numbers are pinned to the Assimp tag fetched above.
cat > "${ASSIMP_DIR}/include/assimp/revision.h" <<'EOF'
#ifndef ASSIMP_REVISION_H_INC
#define ASSIMP_REVISION_H_INC

#define GitVersion 0
#define GitBranch "v6.0.4"
...
#else
#define VER_FILEVERSION_STR         STR(VER_MAJOR) "." STR(VER_MINOR) "." STR(VER_PATCH) "." STR(VER_BUILD) " (Commit generated)"
#endif
