// ModelLoader implementations — split from model_loader.h (Fase 2).
// The header now carries only declarations (plus default arguments).

#include "model_loader.h"

MeshData ModelLoader::createCube(float size, VkDevice device, VkPhysicalDevice physicalDevice, uint32_t (*findMemoryType)(uint32_t, VkMemoryPropertyFlags, VkPhysicalDeviceMemoryProperties*)) {
        MeshData meshData;
        float h = size / 2.0f;
        
        meshData.vertices = {
            // Uniform white vertex colors (like the plane): per-face tint
            // comes from the material only, not baked debug colors.
            ...
            Vertex{{-h,  h, -h}, {1,1,1}, {1,1}, {0,0,-1}},
            Vertex{{ h,  h, -h}, {1,1,1}, {0,1}, {0,0,-1}},
            Vertex{{ h, -h,  h}, {1,1,1}, {0,0}, {1,0,0}},
            Vertex{{ h, -h, -h}, {1,1,1}, {1,0}, {1,0,0}},
            Vertex{{ h,  h, -h}, {1,1,1}, {1,1}, {1,0,0}},
            Vertex{{ h,  h,  h}, {1,1,1}, {0,1}, {1,0,0}},
            Vertex{{-h, -h, -h}, {1,1,1}, {0,0}, {-1,0,0}},
            Vertex{{-h, -h,  h}, {1,1,1}, {1,0}, {-1,0,0}},
            Vertex{{-h,  h,  h}, {1,1,1}, {1,1}, {-1,0,0}},
            Vertex{{-h,  h, -h}, {1,1,1}, {0,1}, {-1,0,0}},
            Vertex{{-h,  h,  h}, {1,1,1}, {0,0}, {0,1,0}},
            Vertex{{ h,  h,  h}, {1,1,1}, {1,0}, {0,1,0}},
            Vertex{{ h,  h, -h}, {1,1,1}, {1,1}, {0,1,0}},
            Vertex{{-h,  h, -h}, {1,1,1}, {0,1}, {0,1,0}},
            Vertex{{-h, -h, -h}, {1,1,1}, {0,0}, {0,-1,0}},
            Vertex{{ h, -h, -h}, {1,1,1}, {1,0}, {0,-1,0}},
            Vertex{{ h, -h,  h}, {1,1,1}, {1,1}, {0,-1,0}},
            Vertex{{-h, -h,  h}, {1,1,1}, {0,1}, {0,-1,0}},
...
MeshData ModelLoader::loadModel(const std::string& path, VkDevice device, VkPhysicalDevice physicalDevice, uint32_t (*findMemoryType)(uint32_t, VkMemoryPropertyFlags, VkPhysicalDeviceMemoryProperties*)) {
...
void ModelLoader::loadModelMultiMesh(
...
void ModelLoader::createBuffers(MeshData& meshData, VkDevice device, VkPhysicalDevice physicalDevice, uint32_t (*findMemoryType)(uint32_t, VkMemoryPropertyFlags, VkPhysicalDeviceMemoryProperties*)) {
        if (device == VK_NULL_HANDLE || physicalDevice == VK_NULL_HANDLE || findMemoryType == nullptr) {
            return;
        }
...
void ModelLoader::cleanupModel(ModelData& modelData, VkDevice device) {
        modelData.clear(device);
    }

void ModelLoader::cleanup(MeshData& meshData, VkDevice device) {
